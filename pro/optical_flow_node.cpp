...
void viconCallback(const nav_msgs::Odometry::ConstPtr& vicon_msg)
{
  ...
  nav_msgs::Odometry gt_pos_odom;
  gt_pos_odom.header.stamp = vicon_msg->header.stamp;
  gt_pos_odom.header.frame_id = "world";
  gt_pos_odom.pose.pose.position.x = position.x();
  gt_pos_odom.pose.pose.position.y = position.y();
  gt_pos_odom.pose.pose.position.z = position.z();
  gt_pos_odom.twist.twist.linear.x = velocity_gt(0);
  gt_pos_odom.twist.twist.linear.y = velocity_gt(1);
  gt_pos_odom.twist.twist.linear.z = velocity_gt(2);
  gt_pos_odom.pose.pose.orientation.w = vicon_msg->pose.pose.orientation.w;
  gt_pos_odom.pose.pose.orientation.x = vicon_msg->pose.pose.orientation.x;
  gt_pos_odom.pose.pose.orientation.y = vicon_msg->pose.pose.orientation.y;
  gt_pos_odom.pose.pose.orientation.z = vicon_msg->pose.pose.orientation.z;
  gt_pos_pub.publish(gt_pos_odom);
...
void imageCallback(const sensor_msgs::Image::ConstPtr& image_msg)
{
  ...
  if (init)
  {
    ...
    temp_velocity(0) = v_camera(0);
    temp_velocity(1) = v_camera(1);
...
int main(int argc, char** argv)
{
  ...
  gt_pos_pub = node.advertise<nav_msgs::Odometry>("/gt_pos_odom", 100);
  // opti_tf_pub_ = node.advertise<nav_msgs::Odometry>("/ekf/opti_tf_odom", 10, true);
  opti_tf_pub_ = node.advertise<geometry_msgs::PointStamped>("/ekf/opti_tf_odom", 100, true);
